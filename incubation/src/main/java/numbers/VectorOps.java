...
public class VectorOps {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(STR."Inputs: a=\{Arrays.toString(a)}, x = \{x}, b = \{Arrays.toString(b)}");
        scalarComputation(a, x, b, sresults);
        System.out.println(STR."Results from scalar = \{Arrays.toString(sresults)}");
        vectorComputation(a, x, b, vresults);
        System.out.println(STR."Results from vector = \{Arrays.toString(vresults)}");
    ...
    static void vectorComputation(double[] a, double x, double[] b, double[] c) {
        ...
        for (; i < upperBound; i += SPECIES.length()) {
			...
			System.out.println(STR."va length = \{va.length()}");
            var vc = va
					.mul(x*x)
                    .add(vb.mul(2));
            vc.intoArray(c, i);
