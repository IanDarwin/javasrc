 ...
 * Tweaked to use java.time.LocalDate in 2022.
 *
...
public class Easter {
	...
	public static final LocalDate findHolyDay(int year) {
		...
		n += 7 - ((d+n)%7);
		if (n>31)			/* E7: */
			return LocalDate.of(year, 4, n-31);	/* April */
		else
			return LocalDate.of(year, 3, n);	/* March */
	}
	...
	public static void main(String[] argv) {
		...
		if (argv.length == 0) {
			...
			LocalDate c = Easter.findHolyDay(thisYear);
			...
			for (String arg : argv) {
				...
				try {
					...
					System.out.println(Easter.findHolyDay(year));
