...
Even https://openbsd.org[OpenBSD] has Java 25 (Thanks Kurt!).
Also sorry if your organization is stuck on an ancient Java due to application server issues.

* I am using IntelliJ Idea for most of my development, and Maven for building, and Jenkins
for automated building. The Ant scripts have been removed.
...
CONSIDER Moving all "compilation error" demos in $js into new "dontcompile/" subdir
